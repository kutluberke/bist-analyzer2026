...
def build_52w_range_chart(
    ...
    """
    Horizontal bullet chart showing current price position in 52-week range.
    """
    if any(v is None or (isinstance(v, float) and np.isnan(v)) for v in [current, low_52w, high_52w]):
        return _empty_chart("52 haftalık aralık verisi eksik")

    rng   = high_52w - low_52w
    pos   = (current - low_52w) / rng * 100 if rng > 0 else 50.0
    color = _GREEN if pos > 60 else (_YELLOW if pos > 35 else _RED)
    ...
    layout = {**_LAYOUT_BASE, "margin": dict(l=10, r=80, t=8, b=8)}
    fig.update_layout(
        **layout,
        barmode="overlay",
        height=90,
        ...
        annotations=[
            dict(x=0,   y=0.5, text=f"₺{low_52w:,.2f}",  showarrow=False,
                 font=dict(color="#9ca3af", size=10), xanchor="left",  yanchor="middle", xref="x", yref="paper"),
            dict(x=100, y=0.5, text=f"₺{high_52w:,.2f}", showarrow=False,
                 font=dict(color="#9ca3af", size=10), xanchor="right", yanchor="middle", xref="x", yref="paper"),
        ],
    )
